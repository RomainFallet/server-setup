...
function ConfigureMattermost() {
  ...
  SetMattermostConfiguration 'EmailSettings.FeedbackName' 'Notifications Mattermost' "${mattermostApplicationName}"
  SetMattermostConfiguration 'EmailSettings.FeedbackEmail' "${mattermostSmtpUserName:?}" "${mattermostApplicationName}"
...
function SetMattermostConfigurationFileContent () {
  ...
  configuration="{
    ...
    \"SqlSettings\": {
      \"DataSource\": \"postgres://${postgresqlUsername}:${postgresqlPassword}@localhost:5432/${postgresqlDatabaseName}?connect_timeout=10\"
    }
  }"
